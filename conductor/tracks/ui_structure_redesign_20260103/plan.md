# Plan: UI Structure and Form Redesign

This plan implements a header-based navigation system and completely redesigns the core forms for properties, bills, and payments to match specific business requirements.

## Phase 1: Foundation & Navigation
- [~] Task: Create View Switcher State & Header Component (TDD)
- [ ] Task: Implement Main Layout with View Switching Logic (TDD)
- [ ] Task: Conductor - User Manual Verification 'Phase 1: Foundation & Navigation' (Protocol in workflow.md)

## Phase 2: Dashboard View Implementation
- [ ] Task: Create Summary Tiles Component (Total Due, Total Paid, Properties Managed) (TDD)
- [ ] Task: Create Alerts/Action Items Component (Bills Due Soon/Overdue) (TDD)
- [ ] Task: Integrate Portfolio Table into Dashboard View (TDD)
- [ ] Task: Conductor - User Manual Verification 'Phase 2: Dashboard View Implementation' (Protocol in workflow.md)

## Phase 3: Properties View & "Add Property" Redesign
- [ ] Task: Implement Properties View with List and "Add Property" Button (TDD)
- [ ] Task: Create Redesigned "Add Property" Modal (Dynamic Tenant fields, Utilities checkboxes) (TDD)
- [ ] Task: Conductor - User Manual Verification 'Phase 3: Properties View & "Add Property" Redesign' (Protocol in workflow.md)

## Phase 4: Bills View & "Add Bill" Redesign
- [ ] Task: Implement Bills View with "Charged in Books" toggle and "Add Bill" Button (TDD)
- [ ] Task: Create Redesigned "Add Bill" Modal (Property dropdown, Date pickers) (TDD)
- [ ] Task: Conductor - User Manual Verification 'Phase 4: Bills View & "Add Bill" Redesign' (Protocol in workflow.md)

## Phase 5: Record Payment Redesign
- [ ] Task: Redesign "Record Payment" Modal (Payment Methods, Calculation logic for Total Paid) (TDD)
- [ ] Task: Implement "Other" Payment Method specification field (TDD)
- [ ] Task: Conductor - User Manual Verification 'Phase 5: Record Payment Redesign' (Protocol in workflow.md)
